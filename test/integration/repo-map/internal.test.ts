...
describe("repository-map operations", () => {
  it("initializes, queries through the short public command, and preserves compatible hidden operations", async () => {
    ...
    await createProgram().parseAsync(["node", "harnix", "internal", "repo-map", "refresh"], { from: "node" });
    expect(JSON.parse(stdout.mock.calls.map((call) => String(call[0])).join(""))).toMatchObject({ scope: "project", status: "refreshed" });
    stdout.mockClear();
    await createProgram().parseAsync(["node", "harnix", "internal", "repo-map", "query", "--query", "billing"], { from: "node" });
