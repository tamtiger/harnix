...
describe("CLI internal-context fast path", () => {
  ...
  it("should_keep_noncanonical_or_invalid_commands_on_the_regular_cli_path", () => {
    expect(canonicalInternalContextPlatform(["node", "harnix", "context", "--platform=antigravity"])).toBeUndefined();
    expect(canonicalInternalContextPlatform(["node", "harnix", "context", "--platform", "unknown"])).toBeUndefined();
    expect(canonicalInternalContextPlatform(["node", "harnix", "context", "--platform", "antigravity", "--extra"])).toBeUndefined();
