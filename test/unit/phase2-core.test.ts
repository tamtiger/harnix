...
import { searchJournal } from "../../src/core/journal/journal.js";
...
describe("phase 2 core", () => {
  it("validates sorted manifest and tracks ownership", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); await writeFile(join(root, "README.md"), "hello\r\n");
  ...
  it("keeps the previous manifest when atomic replacement fails", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); const path = join(root, "manifest.json"); await writeFile(path, "old");
  ...
  it("reconciles managed files while preserving user modifications", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); await writeFile(join(root, "README.md"), "user\n");
  ...
  it("budgets context and discloses omitted sources", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); await writeFile(join(root, "a.md"), "a".repeat(20)); await writeFile(join(root, "b.md"), "b".repeat(20));
  ...
  it("persists and reloads a validated context manifest", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); const task = { generator: "harnix" as const, schemaVersion: 1 as const, taskId: "x", maxCharacters: 10, entries: [], omitted: [] };
  ...
  it("full context bypasses budget while retaining source disclosure", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); await writeFile(join(root, "a.md"), "a".repeat(100));
  ...
  it("applies task transitions and learning threshold", async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), "harnix-")); await saveTask(root, task); await setActiveTask(root, task.id);
  ...
  it("archives only completed tasks and preserves task data", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); const task = taskFixture();
  ...
  it("searches journal newest-first and skips malformed lines", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); const path = join(root, "j.jsonl"); await writeFile(path, `${JSON.stringify({ generator: "harnix", schemaVersion: 1, id: "1", recordedAt: "1", developer: "d", kind: "note", summary: "old", evidenceIds: [] })}\nbad\n${JSON.stringify({ generator: "harnix", schemaVersion: 1, id: "2", recordedAt: "2", developer: "d", kind: "note", summary: "new", evidenceIds: [] })}\n`);
