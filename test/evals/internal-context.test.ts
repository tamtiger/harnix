...
describe("internal context", () => {
  it("returns empty output for an uninitialized project and JSON for Codex", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); expect(await renderInternalContext(root, "kiro")).toBe("");
  ...
  it("bounds Codex hook context and fails closed for corrupt Harnix state", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); await writeConfig(join(root, ".harnix", "config.yaml"), createConfig({ developer: "tam" })); await mkdir(join(root, "docs"), { recursive: true }); await writeFile(join(root, "docs", "large.md"), "x".repeat(10_000));
