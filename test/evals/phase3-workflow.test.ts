...
describe("Phase 3 workflow templates", () => {
  it("seeds canonical workflow and all skills for Kiro and Codex", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); await initializeProject({ root, developer: "tam", yes: true });
  ...
  it("preserves a user-modified managed workflow on rerun", async () => {
    const root = await mkdtemp(join(tmpdir(), "harnix-")); await initializeProject({ root, developer: "tam", yes: true }); await setupPlatforms({ root, platforms: ["kiro"] });
