...
Không tạo task cho câu hỏi, giải thích, standalone read-only review, standalone read-only research, generic status report hoặc yêu cầu không làm thay đổi project. Cũng không tạo task cho một docs-only edit (prose, formatting, hoặc viết/sửa một prompt document) hay một bounded literal-value-only edit (đổi đúng một hằng số numeric/string/enum, tối đa hai file, không đổi behavior/interface/schema) — trừ khi docs edit đó đổi frozen public contract hoặc chứa một material product decision, khi đó phân loại thành Lite hoặc Full như bình thường. Khi người dùng, ngay trong cùng tin nhắn, yêu cầu rõ ràng bỏ qua việc tạo task cho đúng một thay đổi nhỏ mà họ đã đặc tả đầy đủ, hãy sửa trực tiếp và nói rõ là không tạo task — đây là override có phạm vi hẹp cho đúng thay đổi đó, không phải giấy phép thường trực để bỏ qua Lite cho mutation dự án thông thường. Phân loại intent này trước khi đọc `.active`; một unrelated active task không bị load, đổi checkpoint hoặc tiếp tục. Standalone review route tới `harnix-check`; standalone research route tới profile Bypass của `harnix-research`; cả hai trả evidence trực tiếp và không consult active task state. Explicit Harnix-task status có thể dùng bounded public `harnix status` nhưng không resume/mutate task. Nếu review/research dẫn tới yêu cầu sửa hoặc persist project/task artifact, chỉ chuyển sang lifecycle khi người dùng yêu cầu mutation hoặc yêu cầu ban đầu đã bao gồm implementation.
...
.harnix/
  workflow.md
  roadmaps/             # Project-local epic tracking (BẮT BUỘC khi sáng kiến có từ 2 task trở lên)
...
Project-local roadmaps dùng để quản lý các nhóm task theo Epic: khi một sáng kiến, đợt refactor hoặc kế hoạch công việc gồm từ 2 task trở lên (hoặc khi người dùng yêu cầu liên kết), **bắt buộc phải tạo Epic Roadmap** bằng cách gửi `epicId` và payload `epic` (`EpicRecord` schema v1) qua hidden `--save`. ĐẶC BIỆT, lúc khởi tạo Epic Roadmap, **bắt buộc phải khai báo và khởi tạo đầy đủ tất cả các member tasks của Epic ngay từ đầu** (với `epicId` tương ứng) để roadmap phản ánh trọn vẹn toàn bộ kế hoạch ngay từ lúc khởi tạo. Về định dạng ID: Task ID bắt buộc kiểm tra nghiêm ngặt bằng regex `^\d{8}-\d{6}-[a-z0-9]+(?:-[a-z0-9]+)*$` (bắt buộc tiền tố ngày giờ `YYYYMMDD-HHMMSS-`). Epic ID ở tầng validator schema chấp nhận `^[A-Za-z0-9][A-Za-z0-9._-]*$`, nhưng quy ước đồng bộ (best practice) bắt buộc phải luôn đặt tiền tố ngày giờ `YYYYMMDD-HHMMSS-<name>` giống hệt Task ID để bảo đảm tính nhất quán trong toàn bộ hệ thống, giúp sắp xếp và quản lý các file roadmap trong `.harnix/roadmaps/` theo thứ tự thời gian. File `.harnix/roadmaps/<epic-id>.json` là task-owned, và `.harnix/roadmaps/<epic-id>.md` là derived view luôn tự động đồng bộ danh sách members, số lượng trạng thái và task kế tiếp cần thực hiện. Task đơn lẻ độc lập không thuộc chuỗi từ 2 task trở lên không tự tiện gắn `epicId`.
