...
Project-local roadmaps dùng để quản lý các nhóm task theo Epic: khi một sáng kiến, đợt refactor hoặc kế hoạch công việc gồm từ 2 task trở lên (hoặc khi người dùng yêu cầu liên kết), **bắt buộc phải tạo Epic Roadmap** bằng cách gửi `epicId` và payload `epic` (`EpicRecord` schema v1) qua hidden `--save`. ĐẶC BIỆT, lúc khởi tạo Epic Roadmap, **bắt buộc phải khai báo và khởi tạo đầy đủ tất cả các member tasks của Epic ngay từ đầu** (với `epicId` tương ứng) để roadmap phản ánh trọn vẹn toàn bộ kế hoạch ngay từ lúc khởi tạo. File `.harnix/roadmaps/<epic-id>.json` là task-owned, và `.harnix/roadmaps/<epic-id>.md` là derived view luôn tự động đồng bộ danh sách members, số lượng trạng thái và task kế tiếp cần thực hiện. Task đơn lẻ độc lập không thuộc chuỗi từ 2 task trở lên không tự tiện gắn `epicId`.
