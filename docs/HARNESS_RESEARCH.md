...
Khi verification ngày 2026-08-27 mở broad input glob trên chính task này, self-audit phát hiện hai self-reference: exact active `.harnix/tasks/<active-id>/task.json` là nơi workflow append evidence, còn `verification-inputs.json` là sidecar được tạo/cập nhật bởi chính pass; nếu raw-hash, mỗi save tự đổi digest và tạo verification treadmill. Repair nhỏ nhất bỏ đúng hai active workflow-owned raw entry khỏi snapshot vì `@task-contract` đã bind completion-relevant task fields và evidence provenance không thể là input của chính nó. Matching records/sidecars của historical/other task tiếp tục raw-hash; contract hash/payload và top-level sidecar schema v1 giữ nguyên. Regression chạy broad glob end-to-end: persist pass vẫn fresh, nhưng đổi historical sidecar/task content hoặc task contract vẫn làm digest đổi.
