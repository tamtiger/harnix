...
- **Codex:** `$CODEX_HOME/hooks.json` has a nested `UserPromptSubmit` command `harnix context --platform codex`, timeout 5 and `additionalContextLimit: 2500`. Output is valid nested `hookSpecificOutput` or plain developer context according to the current schema; Windows uses a constant launcher form only after shim-resolution smoke evidence.
