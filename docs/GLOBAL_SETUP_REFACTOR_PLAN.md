...
Tài liệu này là kế hoạch triển khai, chưa mô tả behavior đã có trong bản `0.5.0`. Kế hoạch được lập ngày **2026-08-11** từ yêu cầu mới và tài liệu chính thức hiện hành của Kiro, Antigravity và Codex.
...
- Skill, instruction/rule và hook global chỉ kích hoạt workflow khi workspace hiện tại có `.harnix/config.yaml`; ngoài Harnix project chúng phải no-op nhanh, không chặn prompt và không tạo state.
...
- Kiro global IDE hook có từ IDE `1.0.182`. CLI global hook xuất hiện trong V3 early-access của `2.13.0`, còn current standalone JSON format là CLI 3.0 contract. Setup/doctor phải phân biệt IDE support, CLI engine/capability và legacy CLI thay vì chỉ parse một version string hoặc sinh schema cũ. Xem [Kiro IDE 1.0.182](https://kiro.dev/changelog/ide/1-0-182/) và [Kiro CLI 2.13.0](https://kiro.dev/changelog/cli/2-13/).
- Antigravity docs hiện hiển thị Desktop/2.0 `2.6.0`, IDE `2.1.1`, CLI `1.1.11`.
- Codex dùng current official schema tại ngày research; version drift được doctor báo và schema fixture phải được revalidate trước release.
...
- `harnix uninstall --legacy-project-surfaces [--yes]` preview rồi xóa unchanged integration cũ do project manifest chứng minh sở hữu. Flag này mutually exclusive với `--global` và `--purge`; không thêm public command thứ tám.
...
- Doctor cảnh báo Kiro version không hỗ trợ global JSON hook và Harnix workspace hook cũ có thể chạy trùng.
...
- Khi không có context, trả `{ "injectSteps": [] }`. Không trả plain text như adapter cũ.
...
- Doctor chỉ báo `shadowed` khi precedence của installed Antigravity version được xác minh; nếu plugin-vs-workspace precedence không xác định thì báo `precedence-unknown`. File tồn tại không đủ để claim active. Workspace precedence được revalidate từ [Antigravity hooks](https://antigravity.google/docs/hooks) và current product release notes trước khi khóa fixture.
...
- Codex yêu cầu review/trust exact hash của non-managed command hook. Setup phải trả `installed-pending-trust`; người dùng mở `/hooks` để review/trust, và hook mới hoặc bị đổi tiếp tục bị Codex skip cho tới khi trust lại. Harnix không tự dùng `--dangerously-bypass-hook-trust` và doctor không claim `installed` là `active` nếu chưa có bằng chứng trust.

## 8. Internal context và activation guard

Mọi global skill/rule/instruction phải bắt đầu bằng guard tương đương:

1. Tìm initialized project root an toàn từ cwd/workspace event.
...
4. Nếu state hiện hữu nhưng corrupt, không đọc thêm project data; trả warning đã redact và để doctor xử lý.

`harnix internal context` phải:

- Không network, không write, không log prompt/transcript/credential.
- Exit `0` và không output ở non-Harnix repo.
...
4. Explicit legacy cleanup chỉ xóa unchanged entries được project manifest v1 chứng minh sở hữu; modified/untracked luôn preserve.
...
- Antigravity zero/one/multi-root, invocation >0 và shadowed workspace hook.
...
- Kiro version floor, global/workspace duplicate hook và current v1 schema.
...
1. Dùng disposable Windows user/profile hoặc explicit test home.
2. Chạy `harnix setup --kiro --antigravity --codex --dry-run --json` và kiểm logical targets.
3. Apply setup, mở session mới trong từng tool và kiểm skill discovery/hook activation.
...
5. Chạy doctor, uninstall preview, uninstall/apply và verify unrelated global config còn nguyên.
...
| Platform schema tiếp tục drift | High | Dated official fixtures, version/capability detection, doctor warning, release revalidation. |
...
- Doctor phân biệt rõ installed, active, shadowed, drifted, unsupported-version và project-not-initialized.
- Codex chỉ được claim hook active sau review/trust evidence; file presence chỉ chứng minh installed.
