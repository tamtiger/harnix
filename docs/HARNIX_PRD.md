...
- Required pass evidence mang `inputDigest` SHA-256 của canonical task contract, Full PRD/plan và sorted repository input hashes. Nếu một input glob match exact active `.harnix/tasks/<active-id>/task.json`, snapshot bỏ riêng raw file entry đó vì `@task-contract` đã bind completion-relevant task fields; mọi historical/other task record vẫn được raw-hash. Save recompute để chặn race; finish recompute latest required snapshots từ immutable task-owned sidecar v1, không đổi payload/schema.
