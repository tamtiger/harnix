...
export async function setupPlatforms(options: SetupPlatformsOptions): Promise<SetupPlatformsResult> {
  ...
  for (const platform of configured) {
    if (platform === "codex") codexPlan = await prepareCodexSurfaces(options.root, true, config.platforms.includes("codex"));
    if (platform === "antigravity") {
      antigravityPlan = await prepareAntigravitySurface(options.root, true);
  ...
  await writeConfig(configPath, { ...config, platforms: nextPlatforms });
